...
import { clamp, newDrive, ROUTE_LENGTH, speedLimit, stepDrive, type DriveState, type Weather, type Course, COURSES, roadCenter } from "@/lib/driving-engine";

import { useSaved } from "@/lib/storage";
import { RouteArt } from "./lab-art";
...
function paint(ctx: CanvasRenderingContext2D, s: DriveState, width: number, height: number) {
  const night=s.weather==="night", rain=s.weather==="rain", horizon=height*.31, roadWidth=width*.46;
  const centreAt=(t:number)=>width/2+(roadCenter(s.distance+65*(1/Math.max(t,.03)-1),s.course)-roadCenter(s.distance,s.course))*width*.025*t;
  const project=(z:number)=>{const t=1/(1+Math.max(0,z)/65);return {y:horizon+(height-horizon)*t,half:width*.035+roadWidth*t,x:centreAt(t),t};};
  ...
  for(let y=horizon;y<height;y+=2){const t=(y-horizon)/(height-horizon),half=width*.035+roadWidth*t,centre=centreAt(t);ctx.fillStyle=Math.floor(s.distance/6+30/Math.max(t,.01))%2?"#d8d9ce":"#b75a51";ctx.fillRect(centre-half-8*t,y,half*2+16*t,3);ctx.fillStyle=rain?"#3d4b56":night?"#25333e":"#48545e";ctx.fillRect(centre-half,y,half*2,3);}
  ...
  for(let z=Math.floor((s.distance+400)/38)*38;z>s.distance;z-=38){const p=project(z-s.distance);for(const side of [-1,1]){const x=p.x+side*(p.half+32*p.t);if(s.course==="town"){const h=90*p.t,w=46*p.t;ctx.fillStyle=night?"#344858":side<0?"#c6ad8b":"#9ab4b0";ctx.fillRect(x-w/2,p.y-h,w,h);ctx.fillStyle=night?"#f9dda1":"#365465";for(let row=0;row<3;row++)for(let col=0;col<2;col++)ctx.fillRect(x-w*.3+col*w*.4,p.y-h*.83+row*h*.25,w*.18,h*.13);ctx.fillStyle="#53656a";ctx.fillRect(x-w*.6,p.y-h,w*1.2,h*.07);}else{ctx.fillStyle="#674c37";ctx.fillRect(x-3*p.t,p.y-42*p.t,6*p.t,42*p.t);ctx.fillStyle=night?"#17362e":"#345f4c";ctx.beginPath();ctx.moveTo(x,p.y-90*p.t);ctx.lineTo(x-28*p.t,p.y-10*p.t);ctx.lineTo(x+28*p.t,p.y-10*p.t);ctx.fill();}}}
  ...
  for(const checkpoint of [...s.checkpoints].reverse()){const z=checkpoint.distance-s.distance;if(z<0||z>400)continue;const p=project(z),x=p.x-p.half-24*p.t;ctx.strokeStyle="#e6ebcc";ctx.lineWidth=Math.max(1,5*p.t);ctx.beginPath();ctx.moveTo(p.x-p.half,p.y);ctx.lineTo(p.x+p.half,p.y);ctx.stroke();ctx.fillStyle="#b7c4c2";ctx.fillRect(x-2*p.t,p.y-75*p.t,4*p.t,75*p.t);ctx.fillStyle=checkpoint.kind==="stop"?"#b43e3b":"#dcc88c";ctx.fillRect(x-24*p.t,p.y-105*p.t,48*p.t,34*p.t);ctx.fillStyle=checkpoint.kind==="stop"?"#fff":"#23343b";ctx.textAlign="center";ctx.font=`bold ${Math.max(5,14*p.t)}px sans-serif`;ctx.fillText(checkpoint.kind==="stop"?"STOP":String(checkpoint.limit),x,p.y-83*p.t);}
...
export default function DrivingSimulator(){
  ...
  const game=useRef(newDrive()),keys=useRef(new Set<string>()),touches=useRef(new Map<number,string>()),steering=useRef(0),scene=useRef<DrivingScene|null>(null),cockpitRef=useRef(false),qualityRef=useRef("balanced");
  ...
  useEffect(()=>{
    ...
    const lost=()=>{scene.current?.dispose();scene.current=null;game.current.running=false;keys.current.clear();touches.current.clear();steering.current=0;setDisplay({...game.current});setGraphics("2D compatibility mode");};
    ...
    return()=>{disposed=true;surface?.removeEventListener("webglcontextlost",lost);scene.current?.dispose();scene.current=null;};
  },[weather,course,quality]);
  useEffect(()=>{
    let frame=0,previous=0,lastUpdate=0,lastPaint=0;
    ...
    const tick=(now:number)=>{
      ...
      if(now-lastPaint>=(game.current.running?(qualityRef.current==="high"?16:32):160)){
      lastPaint=now;
      if(scene.current){try{scene.current.render(game.current,cockpitRef.current);}catch{scene.current.dispose();scene.current=null;setGraphics("2D compatibility mode");}}
    ...
    const down=(e:KeyboardEvent)=>{if(!game.current.running||(e.target as HTMLElement).isContentEditable||e.altKey||e.ctrlKey||e.metaKey||["INPUT","TEXTAREA","SELECT"].includes((e.target as HTMLElement).tagName))return;if((e.target as HTMLElement).tagName==="BUTTON"&&e.key===" ")return;const action=mapping[e.key]??mapping[e.key.toLowerCase()];if(action){e.preventDefault();keys.current.add(action);}};
  ...
  return <section className={`panel driving-simulator driving-v3 ${expanded?"drive-expanded":""}`} aria-label="Mobile driving simulator">
    ...
    <div className="drive-route-select">{(Object.entries(COURSES) as [Course,typeof COURSES.hills][]).map(([id,data])=><button key={id} aria-pressed={course===id} onClick={()=>{if(id!==course)setGraphics("Loading 3D scene");setCourse(id);reset(weather,id);}}><RouteArt town={id==="town"} training={id==="training"}/><strong>{data.title}</strong><small>{data.description}</small></button>)}</div>
    <div className="row drive-settings"><label>Weather <select value={weather} onChange={e=>{const value=e.target.value as Weather;setGraphics("Loading 3D scene");setWeather(value);reset(value);}}><option value="day">Daylight</option><option value="rain">Rain</option><option value="night">Night</option></select></label><label>Graphics <select value={quality} onChange={e=>{game.current.running=false;setGraphics("Loading 3D scene");qualityRef.current=e.target.value;setQuality(e.target.value);}}><option value="balanced">Balanced / mobile</option><option value="high">Detailed / shadows</option></select></label><button className="btn secondary" disabled={graphics!=="3D active"} onClick={()=>{cockpitRef.current=!cockpit;setCockpit(!cockpit);}}>{cockpit?"Chase camera":"Driver camera"}</button><span className="small muted">{graphics}</span></div>
    <div className="drive-stage" ref={stage}><canvas ref={fallbackCanvas} style={{visibility:graphics==="3D active"?"hidden":"visible"}} aria-label="Compatibility driving view" role="img"/><canvas key={`${quality}-${course}-${weather}`} ref={canvas} style={{visibility:graphics==="3D active"?"visible":"hidden"}} aria-label="3D road, moving traffic and your car" role="img" onContextMenu={e=>e.preventDefault()}/>
      <div className="drive-hud"><div className="speed-dial" style={{"--speed":`${Math.min(100,display.speed/90*100)}%`} as React.CSSProperties}><strong>{Math.round(display.speed)}</strong><small>km/h</small></div><div><strong>{Math.round(display.score)}</strong><small>control score</small></div><div><strong>{speedLimit(weather,display.distance,course)}</strong><small>course limit</small></div></div>
      ...
      <div className="route-mini" aria-label={`${Math.round(display.distance/ROUTE_LENGTH*100)} percent of route completed`}><svg viewBox="0 0 80 150" aria-hidden="true"><path d={`M${Array.from({length:31},(_,i)=>`${40+roadCenter(i*40,course)*1.8},${138-i*4.2}`).join(" L")}`} fill="none" stroke="#80948c" strokeWidth="7"/>{display.checkpoints.map(c=><circle key={c.id} cx={40+roadCenter(c.distance,course)*1.8} cy={138-c.distance/40*4.2} r="4" fill={c.passed?c.success?"#cbef9b":"#ef9c87":"#e9d5a7"}/>)}<circle cx={40+roadCenter(display.distance,course)*1.8} cy={138-display.distance/40*4.2} r="6" fill="#daf4bb" stroke="#29453d" strokeWidth="2"/></svg><small>{Math.round(display.distance/ROUTE_LENGTH*100)}%</small></div>
