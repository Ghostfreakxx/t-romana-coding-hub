...
export default function PwaProvider({children}:{children:React.ReactNode}) {
  ...
  return <Context.Provider value={{installed,ready,failed,canInstall:!!prompt,install,ios}}>{children}
    <div className="pwa-bar no-print" aria-label="App installation and connection">
      ...
      {waiting&&<button className="btn" disabled={updating||!online} onClick={()=>{reloadRequested.current=true;setUpdating(true);waiting.postMessage({type:"SKIP_WAITING"});}}> {updating?"Updating…":online?"Reload to update":"Connect to update"}</button>}
    </div>
  </Context.Provider>;
}
