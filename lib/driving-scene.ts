...
export function createDrivingScene(canvas: HTMLCanvasElement, state: DriveState, highQuality: boolean): DrivingScene {
  ...
  const box=new THREE.BoxGeometry(1,1,1), cone=new THREE.ConeGeometry(1,1,7), cylinder=new THREE.CylinderGeometry(1,1,1,10);
  geometries.push(box,cone,cylinder);
  function mesh(geometry:THREE.BufferGeometry,mat:THREE.Material,parent:THREE.Object3D,x:number,y:number,z:number,sx:number,sy:number,sz:number) {
    const m=new THREE.Mesh(geometry,mat);m.position.set(x,y,z);m.scale.set(sx,sy,sz);m.castShadow=highQuality;m.receiveShadow=highQuality;parent.add(m);return m;
  }
  ...
  const roadVertices:number[]=[],roadIndices:number[]=[];
  for(let d=-30,i=0;d<=1850;d+=5,i++){roadVertices.push(center(d)-6,.02,-d,center(d)+6,.02,-d);if(i)roadIndices.push((i-1)*2,(i-1)*2+1,i*2,(i-1)*2+1,i*2+1,i*2);}
  const roadGeometry=new THREE.BufferGeometry();roadGeometry.setAttribute("position",new THREE.Float32BufferAttribute(roadVertices,3));roadGeometry.setIndex(roadIndices);roadGeometry.computeVertexNormals();geometries.push(roadGeometry);const road=new THREE.Mesh(roadGeometry,roadMat);road.receiveShadow=true;scene.add(road);
  ...
  for(let d=-20;d<1650;d+=24){for(const side of [-1,1]){
    ...
    else{const tree=new THREE.Group();tree.position.set(x,0,-d);scene.add(tree);mesh(box,bark,tree,0,1.5,0,.65,3,.65);mesh(cone,leaves,tree,0,4.5,0,3.3,7,3.3);mesh(cone,leaves,tree,0,7,0,2.3,4,2.3);}
  ...
  for(let d=0;d<1700;d+=170){for(const side of [-1,1])mesh(cone,material(d%340?"#58766e":"#75958c"),scene,side*(100+d%60),25,-d,90,80,100);}
  function vehicle(color:string){const group=new THREE.Group();const paint=material(color);
    mesh(box,paint,group,0,.65,0,1.75,.65,3.5);mesh(box,paint,group,0,1.25,.15,1.52,.65,1.8);
    const wind=mesh(box,glass,group,0,1.29,-.77,1.35,.49,.05);wind.rotation.x=.2;mesh(box,glass,group,0,1.29,1.07,1.35,.44,.04);
    for(const side of [-1,1]){mesh(box,glass,group,side*.77,1.29,.14,.025,.46,1.5);for(const z of [-1.15,1.12]){const tire=mesh(cylinder,rubber,group,side*.9,.36,z,.36,.22,.36);tire.rotation.z=Math.PI/2;const hub=mesh(cylinder,chrome,group,side*1.02,.36,z,.18,.025,.18);hub.rotation.z=Math.PI/2;}mesh(box,redLight,group,side*.57,.7,1.77,.32,.17,.04);mesh(box,headLight,group,side*.57,.7,-1.77,.35,.15,.04);}
  ...
  },dispose(){scene.traverse(o=>{if(o instanceof THREE.Mesh)o.geometry.dispose();});materials.forEach(m=>m.dispose());geometries.forEach(g=>g.dispose());textures.forEach(t=>t.dispose());renderer.dispose();}};
}
